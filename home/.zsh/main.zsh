...
if [[ $BATTERY -gt 0 ]] ; then
    ...
    if [[ -z "$PERCENT" ]] ; then
        ...
        if [[ "$PERCENT" -lt 15 ]] ; then
            PERCENT="[⚡%F{red}${PERCENT}%%%f]"
        elif [[ "$PERCENT" -lt 35 ]] ; then
            PERCENT="[⚡%F{yellow}${PERCENT}%%%f]"
        else
            PERCENT="[⚡%F{green}${PERCENT}%%%f]"
        fi
    fi
fi
}
...
function prompt_grml_precmd () {
    ...
    left_items=(newline
                datetime tty shell-level jobs plat version battery newline
                user at host colon path rc history vcs percent)
    right_items=()
